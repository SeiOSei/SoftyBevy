...
<body>
    <div class="headercontainer">
        <header class="Dashheader">
            <span class="headerspan"><a class="a1" href="Dashboard-Loggedin-customer.php">SoftyBevy</a></span>
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a href="Dashboard-Loggedin-Customer.php" class="nav-link active" aria-current="page">Home</a>
                </li>
                <li class="nav-item">
                    <a href="Cart-Customer.php" class="nav-link">Cart</a>
                </li>
                <li class="nav-item">
        ...
        <div class="product-grid" style="display: flex; flex-wrap: wrap; gap: 20px; justify-content: flex-start; padding-top: 35px;">
            <?php if(isset($products) && !empty($products)) {
                foreach ($products as $product) {
                    ...
                    echo '<form action="Cart-Customer.php" method="post">';
                    echo '<input type="hidden" name="product_id" value="'.$product['productID'].'">';
                    echo '<input type="hidden" name="product_name" value="'.$product['productName'].'">';
                    echo '<input type="hidden" name="price" value="'.$product['Price'].'">';
                    echo '<input type="hidden" name="image" value="'.$product['image'].'">';
                    echo '<button type="submit" class="btn" name="add_product" style="margin-top: 5px; margin-bottom: 10px; padding: 5px; background-color: skyblue; color: white; border: none; border-radius: 5px; cursor: pointer;">Add to Cart</button>';
                    echo '</form>';
